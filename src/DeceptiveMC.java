...
public class DeceptiveMC extends Graph {
	...
	private HashMap<String,Integer> create_map(int number_bits,int slope, int delta){
		HashMap<String,Integer> result = new HashMap<String,Integer>();
		GrayCode.gray("", number_bits, result);
		
		// Trap Markov chain score with variable delta and mu
		HashSet<Integer> possible_scores = new HashSet<Integer>();
		for (int i=0;i<= number_bits;++i){
			possible_scores.add(i);		//All possible scores for TMC
		}
		
		int max_score=max_set(possible_scores);
		...
		for (int k=slope; k>delta;k--){
			int min_score=min_set(possible_scores);
	...
	protected int max_set(HashSet<Integer> set){
		int max=0;
		for(Integer i: set){
			if (i> max){
				max=i;
	...
	protected int min_set(HashSet<Integer> set){
		int min=100;
		for(Integer i: set){
			if (i < min){
				min=i;
