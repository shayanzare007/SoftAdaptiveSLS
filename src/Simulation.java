...
public class Simulation {
	...
	public int Stochastic_Local_Search(){
		...
		while(t<MAX_TRIES){
			...
			int init_score = graph.get(current);
		...
		while(t<MAX_TRIES){
			...
			while (!restart){
				...
				if (random <= restart_probability*1000) {
					...
					Pr= Pr - Pr * gamma/2;
					restart_probability= restart_probability - restart_probability * gamma/2;
				}
				else {
					random = randomGen.nextInt(100)+1;
	...
	public static void main(String[] args) {
		...
		double total_table[][][]=new double[50][9][51];
		...
		int delta=0;
		for(int slope=1;slope<10;slope+=1){
